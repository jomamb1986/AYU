# <!-- este es el codigo COMPLETO y ACTUALIZADO para app/routes/cylinders.py PARTE 1 -->
...
from app.forms import CreateCylinderForm, UpdateCylinderStatusForm, BatchProcessForm, EditCylinderForm
...
def index():
    ...
    base_query = Cylinder.query.filter(Cylinder.esta_activo == True) # FILTRO AÑADIDO
    if query_term:
    ...
    if query_term: next_url_params['q'] = query_term
    ...
    if query_term: prev_url_params['q'] = query_term
...
def create_cylinder():
    ...
    if form.validate_on_submit():
        cylinder = Cylinder(
            ...
            # esta_activo es True por defecto en el modelo
        )
...
def view_cylinder(cylinder_id):
    cylinder = Cylinder.query.get_or_404(cylinder_id) # Obtener siempre, activo o no

    if not cylinder.esta_activo and (not current_user.is_authenticated or current_user.role != 'admin'):
        flash('Este botellón está inactivo y solo los administradores pueden ver sus detalles.', 'warning')
        return redirect(url_for('cylinders.index'))
...
def update_cylinder_status(cylinder_id):
    cylinder = Cylinder.query.filter_by(id=cylinder_id, esta_activo=True).first_or_404()
...
def delete_cylinder(cylinder_id):
    ...
    if not cylinder.esta_activo:
        flash(f'El botellón {cylinder.cylinder_id_tag} ya se encuentra inactivo.', 'info')
        return redirect(url_for('cylinders.index'))
    cylinder.esta_activo = False
    historial_desactivacion = HistorialMovimiento(
        cylinder_id=cylinder.id,
        user_id=current_user.id,
        tipo_movimiento="desactivado",
        fecha_movimiento=datetime.utcnow(),
        nota=f"Botellón marcado como inactivo/eliminado por {current_user.username}."
    )
    db.session.add(historial_desactivacion)
    try:
        db.session.commit()
        flash(f'Botellón {cylinder.cylinder_id_tag} ha sido marcado como inactivo (eliminado lógicamente) y el movimiento registrado.', 'success')
    except Exception as e:
        db.session.rollback()
        flash(f'Error al marcar el botellón como inactivo: {str(e)}', 'danger')
    return redirect(url_for('cylinders.index'))
...
def edit_cylinder(cylinder_id):
    cylinder = Cylinder.query.get_or_404(cylinder_id)
    form = EditCylinderForm(obj=cylinder if request.method == 'GET' else None)

    if form.validate_on_submit(): # Implica request.method == 'POST'
        old_values = {
            'marca': cylinder.marca,
            'color': cylinder.color,
            'numero_serie': cylinder.numero_serie,
            'status': cylinder.status,
            'esta_activo': cylinder.esta_activo
        }
        ...
        if new_status_from_form == 'available':
            cylinder.delivery_date = None
            cylinder.reception_date = None
            if not cylinder.esta_activo: # Reactivar si estaba inactivo y se pone disponible
        ...
        if old_values['esta_activo'] != cylinder.esta_activo: changes_summary.append(f"Actividad: '{'Inactivo' if old_values['esta_activo']==False else 'Activo'}' -> '{'Activo' if cylinder.esta_activo==True else 'Inactivo'}'")

        nota_historial = "Actualización de datos del botellón."
        if changes_summary: nota_historial += " Cambios: " + "; ".join(changes_summary) + "."
        else: nota_historial += " Se guardaron los datos (sin cambios detectables)."
        if form.nota_edicion.data: nota_historial += f" Nota adicional: {form.nota_edicion.data}"
        ...
        try:
            db.session.commit()
            flash('Datos del botellón actualizados y edición registrada en historial.', 'success')
        except Exception as e:
            db.session.rollback()
            flash(f'Error al actualizar botellón: {str(e)}', 'danger')
        return redirect(url_for('cylinders.view_cylinder', cylinder_id=cylinder.id))
